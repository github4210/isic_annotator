    ...
    if g.user is not None:
        ...
        if request.method == 'POST':
            ...
            if image_from_db:
                ...
                response_dict['status'] = "Database error, could not save annotation"
...
def fill():
    ...
    if request.method == 'GET':
        ...
        result = fillImageGeoJSON(json.loads(request.data))
        msg['result'] = result
...
def segment():
    ...
    msg = {}
    msg['operation'] = 'segment'

    if request.method == 'GET':
        pass
    elif request.method == 'POST':

        result = segmentImage(json.loads(request.data))
        msg['features'] = result
